...
extern int n[10];
float RANGING_DISTANCE = 0,test = 0;
queue ADC3_value;
...
void adc_update_info()
{
	ElemType e;
	int sum;
	float length = 1;
	
	if(TRUE == isFullQueue(&ADC3_value))
	{
		deQueue(&ADC3_value,&e);
		enQueue(&ADC3_value,ADC_GetConversionValue(ADC3));
	}
	else enQueue(&ADC3_value,ADC_GetConversionValue(ADC3));
	
	queueSum(&ADC3_value,&sum);
//	sum = sum;
//	ADC3_value.pBase[0]=ADC3_value.pBase[0];
//	ADC3_value.pBase[1]=ADC3_value.pBase[1];
//	ADC3_value.pBase[2]=ADC3_value.pBase[2];
//	ADC3_value.pBase[3]=ADC3_value.pBase[3];
	length = (int)getQueueLen(&ADC3_value);
	if(0 == length)length = 1;
	
	RANGING_DISTANCE = 995.0f * ((sum / length) / 4096.0f * 3.1f * 3.333333f) + 50.0f;  
}
